...
const PeriodeList = () => {
  ...
  const handleDelete = (periodeNama) => {
    Swal.fire({
      title: `Hapus periode "${periodeNama}"?`,
      ...
      if (result.isConfirmed) {
        try {
          await deleteData('periode', periodeNama); // pastikan endpoint delete sesuai
          setPeriodeList((prev) => prev.filter((item) => item.PeriodeNama !== periodeNama));
  ...
  const filteredData = periodeList.filter((item) =>
    item.PeriodeNama.toLowerCase().includes(searchTerm.toLowerCase())
  );
                ...
                {currentItems.length > 0 ? (
                  currentItems.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>{item.PeriodeNama}</td>
                      <td className="d-flex gap-1">
                        <button
                          className="btn btn-danger btn-sm"
                          onClick={() => handleDelete(item.PeriodeNama)}
                        >
